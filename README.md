...
모든 편집은 **"교재 관리"** 화면(사이드바 메뉴, 학생 계정에는 "내 교재"로 표시)에서만
합니다. 단원 목록을 왼쪽에 두고 오른쪽에서 문장/단어/문법/문제를 표 형태로 관리하며,
단원 추가·정보 수정·삭제, 문장/단어/문제 일괄 추가(AI로 만든 콘텐츠 붙여넣기 포함)도
이 화면에서 합니다. 이 메뉴에는 자기가 쓰기 권한을 가진 교재(교사는 담당 클래스 교재,
학생은 자기 개인 교재)만 나타납니다.

학습 화면(본문·단어·문법·연습문제 탭)은 순수하게 읽고 푸는 용도이며, 쓰기 권한이 있는
교재를 보고 있을 때는 헤더에 **"편집하기"** 버튼이 나타나 곧바로 교재 관리 화면의 해당
단원으로 이동할 수 있습니다. (예전에는 학습 화면에도 자체 인라인 편집 모드가 있었지만,
교재 관리 화면과 기능이 겹쳐서 하나로 통합했습니다.)
...
- 학생이 자기 개인 교재를 처음부터 직접 만들 수도 있음("내 교재" 화면에서 단원 생성)
- 권한 기반 편집 UI(쓰기 권한이 있는 교재만 "교재 관리"/"내 교재" 화면에 나타남)
